...
export function mapBadgeFormatter(bytes: Uint8Array, pool: PoolNode[], profile: WorldProfile,
  ...
  const parts=fixed?null:Object.fromEntries(['Minor','Major'].map(kind=>{
  ...
  return marker=>{
    if (marker.tag===15 && marker.symbol==='$star') return {text:starText,size:Math.fround(fixed?89.6:67.2)};
    ...
    if (!parts) return {text:String(value),size:Math.fround(57.6)};
