...
export async function extractMapImages(
  ...
  const binding = (name: string, tag: number) => {
    const b = data.bindings[name];
    if (!b || b.tag !== tag) throw Error(`missing map image binding ${name}`);
    return decodeMapBinding(bytes, pool, profile, b).value as number;
  };
  ...
  const atlas = (await imageLevels(binding('fleckAtlas',71))).sort((a,b)=>b.width-a.width);
  ...
  for (const [name,key] of [['round','labelRound'],['panel','labelPanel'],['entity','entityPanel'],
    ...
    const panel=name==='round' || name==='panel' || name==='entity';
