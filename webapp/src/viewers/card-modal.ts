...
export async function openCardModal(app: any, model: { id: string; name?: string }): Promise<void> {
  ...
  for (const s of SCALES) scaleSel.appendChild(el('option', { value: String(s), text: `${s}x` }));
  scaleSel.value = '4';
  ...
  const updateDims = () => {
    ...
    const w = icon ? ICON_SIZE : CARD_WIDTH, h = icon ? ICON_SIZE : CARD_HEIGHT;
    dims.textContent = `${w * scale}×${h * scale}`;
  };
  let generation = 0;
  const refresh = async () => {
    updateDims();
