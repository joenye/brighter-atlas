// opus-decoder 0.7.11 — https://github.com/eshaz/wasm-audio-decoders (MIT). libopus
// (xiph/opus master ~1.5.2, BSD-3-Clause) compiled to WASM and embedded below as a
// yEnc+DEFLATE string (decoded at runtime by the bundled simple-yenc 1.0.4, MIT) — fully
// self-contained: no network, no DOM; runs in Node >= 18 and in browser (module) workers.
// Built from the npm tarballs opus-decoder-0.7.11 + @wasm-audio-decoders/common 9.0.7 with
//   esbuild --bundle --format=esm --platform=neutral --alias:@eshaz/web-worker=<empty stub>
// exporting ONLY the synchronous OpusDecoder (the OpusDecoderWebWorker wrapper needs a Worker
// polyfill and is unused here — the viewer runs decoding inside its own worker already).
// NOTE: the official dist/opus-decoder.min.js was NOT used: its minifier mangles the
// constructor option keys (channels/streamCount/...), silently ignoring them.
